...
resource "null_resource" "remove_replication_global" {
  ...
  provisioner "local-exec" {
    command = "aws docdb --region ${var.global_region[count.index]} remove-from-global-cluster --db-cluster-identifier arn:aws:rds:${var.global_region[count.index]}:${var.aws_account_id}:cluster:${var.name} --global-cluster-identifier ${var.global_identifier}"
  }
}
...
resource "null_resource" "remove_source_global" {
  ...
  provisioner "local-exec" {
    command = "aws docdb --region ${var.region} remove-from-global-cluster --db-cluster-identifier arn:aws:rds:${var.region}:${var.aws_account_id}:cluster:${var.name} --global-cluster-identifier ${var.global_identifier}"
  }
